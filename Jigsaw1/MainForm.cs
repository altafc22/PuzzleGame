...
namespace MinThantSin.OpenSourceGames
{
    public partial class MainForm : Form
    {
        ...
        /// </summary>
        public List<PieceCluster> _clusters = new List<PieceCluster>();
        ...
        public int puzzle_height { get; set; }
        public int puzzle_width { get; set; }
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            gtClass = new GtClass();
            _previousClientWidth = puzzle_width;
            _previousClientHeight = puzzle_height;
            _puzzlePictureWidth = puzzle_width;
            _puzzlePictureHeight = puzzle_height;

            //startCountdown();
            Console.WriteLine("Image Recieved from Game Screen : " + imagePath);
            loadImage(imagePath);
            //lblCountDown.Font = new Font("Digital-7", 14, FontStyle.Bold);
        }
        ...
        {
            if (_previousClientWidth != puzzle_width || _previousClientHeight != puzzle_height)
            {
                _previousClientWidth = puzzle_width;
                _previousClientHeight = puzzle_height;
            ...
            if (_canMovePiece)
            {
                ...
                if (_clusters.Count == 1)
                {
                    if (_victoryAnnounced == false)
                    {
                        ...
                        gtClass.playAudio("win.wav");
                        _victoryAnnounced = true;
                        WinnerForm winnerForm = new WinnerForm();
                        winnerForm.imagePath = imagePath;
                        winnerForm.result = "winner";
                        winnerForm.ShowDialog();
                        Application.OpenForms["GameScreen"].Close();
                        //this.Close();
        ...
        private void loadImage(string imagePath)
        {
            ...
            if (!String.IsNullOrEmpty(openFileDialog1.FileName))
            {
                ...
                _sourcePicture = new Bitmap(imagePath);
                ...
                {
                    Console.WriteLine(_puzzlePictureWidth + "--------" + _puzzlePictureHeight);
        ...
        private ResponseMessage CreateJigsawPuzzle()
        {
            ...
            int boardWidth = puzzle_width;
            int boardHeight = puzzle_height;
        ...
        private ResponseMessage DisplayJigsawPuzzle(bool showGhostPicture)
        {
            ...
            using (Graphics gfx = Graphics.FromImage(_background))
            {
                if (File.Exists(GameSettings.BACKGROUND_PICTURE_NAME))
                {
                    ...
                    Bitmap tileImage = new Bitmap(GameSettings.BACKGROUND_PICTURE_NAME);
